...
jobs:
  build-and-push:
    ...
    steps:
      ...
      - id: push-to-gar
        uses: grafana/shared-workflows/actions/push-to-gar-docker@push-to-gar-docker/v0.7.1
        with:
...
| `delete_credentials_file` | Boolean | Delete the credentials file after the action is finished. If you want to keep the credentials file for a later step, set this to false.                                                                                                                                    | true                               |
...
| `environment`             | String  | Environment for pushing artifacts (can be either dev or prod).                                                                                                                                                                                                             | dev                                |
| `file`                    | String  | Path and filename of the dockerfile to build from. (Default: `{context}/Dockerfile`)                                                                                                                                                                                       |                                    |
| `image_name`              | String  | Name of the image to be pushed to GAR.                                                                                                                                                                                                                                     |                                    |
| `labels`                  | String  | List of custom labels to add to the image as metadata (see the [metadata-action](https://github.com/docker/metadata-action?tab=readme-ov-file#inputs)) for details                                                                                                         |                                    |
...
| `push`                    | String  | Whether to push the image to the registry.                                                                                                                                                                                                                                 | ${{ github.event_name == 'push' }} |
| `registry`                | String  | Google Artifact Registry to store docker images in.                                                                                                                                                                                                                        | us-docker.pkg.dev                  |
| `repository_name`         | String  | Override the 'repo_name' which is included as part of the GAR repository name. Only necessary when the GAR includes a repo name that doesn't match the GitHub repo name. required: false                                                                                   |                                    |
...
| `ssh`                     | String  | List of SSH agent socket or keys to expose to the build ([more about ssh for docker/build-push-action](https://github.com/docker/build-push-action?tab=readme-ov-file#inputs))                                                                                             |                                    |
