...
| `enforce-hardened-dockerfile` | Boolean | The Dockerfile is always scanned for baseline hardening requirements. When `true`, policy violations fail the build; when `false` (the default), violations are reported as warnings only.                             |
