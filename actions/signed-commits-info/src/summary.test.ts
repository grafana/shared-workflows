...
describe("buildSummary", () => {
  ...
  test("renders a heading", () => {
    ...
    expect(core.summary.stringify()).toContain(
      "<h1>Signed commits report</h1>",
    );
  });
  ...
  test("shows the success message when everything is verified", () => {
    ...
    expect(out).toContain(
      "Checked 3 commits between <code>main</code> and <code>feat</code>.",
    );
  ...
  test("uses plural form when multiple commits are unverified", () => {
    buildSummary(
      5,
      [
        makeCommit({ sha: "a".repeat(40) }),
        makeCommit({ sha: "b".repeat(40) }),
      ],
      "main",
      "feat",
    );
    expect(core.summary.stringify()).toContain(
      "2 commits could not be fully verified:",
    );
  });
...
describe("buildSummaryTableRow", () => {
  test("returns four non-empty string cells even when author fields are missing", () => {
    const row = buildSummaryTableRow(
