...
// Destructured so the local binding does not shadow the `crypto` global that
// Node exposes for the Web Crypto API.
const { createHash, randomBytes } = require("node:crypto");

// Default timeout (ms) for outbound HTTP requests. Without an explicit
// timeout, a half-open socket (connection established but no response) would
// leave the `fetch` promise pending forever, so the retry logic never fires
// and the job only ends when its `timeout-minutes` kills it. `AbortSignal.timeout`
// rejects the request with a TimeoutError instead, letting the caller retry.
