...
const parseInputs = () => {
  ...
  // GITHUB_WORKFLOW_REF is part of the Vault role name. If it is missing the
  // role would be built with an empty ref hash and silently resolve to the
  // wrong (or a non-existent) role, so fail loudly instead — same as the
  // GITHUB_REPOSITORY check above.
  const workflowRef = process.env.GITHUB_WORKFLOW_REF || "";
  if (!workflowRef) {
    throw new Error("GITHUB_WORKFLOW_REF is not set or is malformed.");
  }

  return { permissionSet, vaultInstance, apps, repositoryName, workflowRef };
};
...
const main = async () => {
  const { permissionSet, vaultInstance, apps, repositoryName, workflowRef } =
    parseInputs();
