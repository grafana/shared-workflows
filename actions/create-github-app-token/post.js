...
const revokeVaultToken = async ({ vaultUrl, vaultToken, proxyJwt }) => {
  // This uses a custom retry loop instead of `lib.retry` because it needs to
  // branch on the HTTP status: a 403/404 means the token is already gone and
  // must short-circuit without retrying, whereas `lib.retry` only knows how to
  // retry on a thrown error.
